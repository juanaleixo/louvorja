<template>
  ...
  <!-- Layer 0b: Wallpaper do settings (com fade) -->
  <Transition name="fade">
    <div
      v-if="!curBg.active"
  ...
  <div v-if="projActive" class="layer-projection">
    <Slide
      ...
      class="layer-slide"
    />
    <img v-else-if="projType === 'file_image'" :src="projUrl" class="layer-file" alt="" />
    <video
...
import { MAIN_BACKGROUND_ID } from "@/types/db/settings/BackgroundSettings";
...
async function reloadWallpaper(): Promise<void> {
  const s = await getSetting<MainSettings>(MAIN_BACKGROUND_ID).catch(() => null);
  if (s) {
